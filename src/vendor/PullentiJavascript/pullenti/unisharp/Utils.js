...
class Utils {  
    ...
    static startsWithString(str, sub, nocase = false) {
        ...
        return start.toUpperCase() == sub.toUpperCase();
    }
    ...
    static endsWithString(str, sub, nocase = false) {
        ...
        return end.toUpperCase() == sub.toUpperCase();
    }
	...
	static parseInt(str) {
        while(str[0] == '0' && str.length > 1) str = str.substring(1);
		return Number(str);
	}
    ...
    static gzipWrapper(stream, mode) {
        if(mode != 'r') throw new Error("Supports only r-mode (decompress)");
