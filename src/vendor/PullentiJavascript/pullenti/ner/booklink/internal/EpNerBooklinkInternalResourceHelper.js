...
const EpNerBooklinkPropertiesResources = require("./../properties/EpNerBooklinkPropertiesResources");
...
class EpNerBooklinkInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerBooklinkPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerBooklinkPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerBooklinkPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerBooklinkInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerBooklinkInternalResourceHelper