...
const EpNerBusinessPropertiesResources = require("./../properties/EpNerBusinessPropertiesResources");
...
class EpNerBusinessInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerBusinessPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerBusinessPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerBusinessPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerBusinessInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerBusinessInternalResourceHelper