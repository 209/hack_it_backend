...
const EpNerCorePropertiesResources = require("./../properties/EpNerCorePropertiesResources");
...
class EpNerCoreInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerCorePropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerCorePropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerCorePropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerCoreInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerCoreInternalResourceHelper