...
const EpNerGoodsPropertiesResources = require("./../properties/EpNerGoodsPropertiesResources");

/**
 * Это для поддержки получения встроенных ресурсов
 */
class EpNerGoodsInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerGoodsPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerGoodsPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerGoodsPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerGoodsInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerGoodsInternalResourceHelper