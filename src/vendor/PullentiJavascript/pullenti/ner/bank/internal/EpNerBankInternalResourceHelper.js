...
const EpNerBankPropertiesResources = require("./../properties/EpNerBankPropertiesResources");
...
class EpNerBankInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerBankPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerBankPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerBankPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerBankInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerBankInternalResourceHelper