...
const EpNerOrgPropertiesResources = require("./../properties/EpNerOrgPropertiesResources");
...
class EpNerOrgInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerOrgPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerOrgPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerOrgPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerOrgInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerOrgInternalResourceHelper