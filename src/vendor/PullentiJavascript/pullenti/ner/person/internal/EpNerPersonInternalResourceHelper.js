...
const EpNerPersonInternalPropertiesResources = require("./properties/EpNerPersonInternalPropertiesResources");
...
class EpNerPersonInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerPersonInternalPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerPersonInternalPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerPersonInternalPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerPersonInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerPersonInternalResourceHelper