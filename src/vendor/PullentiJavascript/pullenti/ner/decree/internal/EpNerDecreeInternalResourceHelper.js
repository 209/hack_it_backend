...
const EpNerDecreeInternalPropertiesResources = require("./properties/EpNerDecreeInternalPropertiesResources");

/**
 * Это для поддержки получения встроенных ресурсов
 */
class EpNerDecreeInternalResourceHelper {
    ...
    static get_bytes(name) {
        ...
        let names = EpNerDecreeInternalPropertiesResources.getNames();
        for (const n of names) {
            if (Utils.endsWithString(n, name, true)) {
                try {
                    let inf = EpNerDecreeInternalPropertiesResources.getResourceInfo(n);
                    if (inf === null) 
                        continue;
                    let stream = EpNerDecreeInternalPropertiesResources.getStream(n); 
                    try {
    ...
    static get_string(name) {
        let arr = EpNerDecreeInternalResourceHelper.get_bytes(name);
        if (arr === null) 
            return null;
...
module.exports = EpNerDecreeInternalResourceHelper